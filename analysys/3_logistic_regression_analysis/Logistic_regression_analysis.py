...
def flatten_features(minute_data):
    ...
    for key in ["none", "first", "second", "third", "first-second", "first-third", "second-third", "first-second-third"]:
        features.append(int(minute_data['situation_features']['runner_status'].get(key, False)))
    # is_goahead_runner_on_base
    features.append(int(minute_data['situation_features'].get('is_goahead_runner_on_base', False)))
    # score_difference
    for key in ["minus_less_3", "minus_2", "minus_1", "tie", "plus_1", "plus_2", "plus_more_3"]:
        features.append(int(minute_data['situation_features']['score_difference'].get(key, False)))
    # inning_phase
    features.append(int(minute_data['situation_features']['inning_phase'].get('early', False)))
    # diff_time
...
if len(np.unique(y)) > 1:
    ...
    print("Warning: Only one class present in y. Logistic regression cannot be trained.")
